...
typedef struct ListaGra ListaGra;
...
//------------------------ESTRUCTURA PROCESAMIENTO DE CARTAS-------------------------------



typedef struct ListaPro ListaPro;
typedef struct NodoPro NodoPro;

struct NodoPro
{
	int idAyudantePro, anioPro, idNinoPro;
	char decision [LONG];
	NodoAS *siguiente;
	NodoAS *anterior; 
};
...
modificarJuguete(struct node* actual, char juguete[],char opcion[]){
   ...
   while(actual != NULL){
	...
	printf("%s", actual->nombre);
      ...
      else if(strcmp(juguete, actual->nombre)> 0) {
...
void liberarListaAS(ListaDobleAS *L)
{
	NodoAS *n, *aux;
	if(L->inicio == NULL)
		return;
...
int esIdValido (const ListaDobleAS *L, int id){
	NodoAS *auxiliar;
...
void ingresarVertice(){
	...
	scanf("%d",&nuevo->idVertice);
	getchar(); //borrar basura
	
	printf("Ingrese el nombre del lugar ");
	gets(nuevo->nombreLugar);
...
void insertarPila(Vertice* aux){
	ListaGra* nuevo=(ListaGra*)malloc(sizeof(ListaGra));
	nuevo->lugar =aux;
	nuevo->sig=NULL;
	if(ini==NULL){
		ini=nuevo;
...
void eliminarVertice(){
	...
	char final[LONG];
	fflush(stdin);
	printf("Indique el que desea eliminar: ");
	gets(final);
...
void ingresarCartas (ListaCartaNino *LC, const ListaR *L, const struct node* root ){
	...
	printf("\nQuieres Agregar los juguetes a tu carta o la lista de deseos? \n1. CARTA PARA SANTA \n2. LISTA DE DESEOS\n\n");
	scanf("%d",&cual);
	getchar(); //borrar basura
...
void modificarCarta (ListaCartaNino *LC, int idModi, int anioM, int pModiSelec, char pModiJugue[] ){
	NodoCarta *aux;
	
	for (aux=LC->inicio; aux!= NULL; aux=aux->siguiente){
		if(aux->idNinoC == idModi && aux->anio == anioM){
			
			if (pModiSelec == 1){
				strcpy(aux->juguete1,pModiJugue);
			}
			else if(pModiSelec == 2){
				strcpy(aux->juguete2,pModiJugue);
			}
			else if(pModiSelec == 3){
				strcpy(aux->juguete3,pModiJugue);
			}
			else if(pModiSelec == 4){
				strcpy(aux->juguete4,pModiJugue);
			}
			else if(pModiSelec == 5){
				strcpy(aux->juguete5,pModiJugue);
			}
			else if(pModiSelec == 6){
				strcpy(aux->juguete6,pModiJugue);
			}
			else if(pModiSelec == 7){
				strcpy(aux->juguete7,pModiJugue);
			}
			else if(pModiSelec == 8){
				strcpy(aux->juguete8,pModiJugue);
			}
			else if(pModiSelec == 9){
				strcpy(aux->juguete9,pModiJugue);
			}
			else if(pModiSelec == 10){
				strcpy(aux->juguete10,pModiJugue);
			}
			else{
				printf("Debe ingresar un codigo de seleccion valido");
			}
...
int buscarCarta (const ListaCartaNino *LC, int idCarta, int anioCarta){
	NodoCarta *i;
	
	for (i=LC->inicio; i!= NULL; i=i->siguiente){
		if(i->idNinoC == idCarta && i->anio == anioCarta){
			return 1; }
	}
	return 0;
}

void editarCarta(ListaCartaNino *LC, const struct node* root ){
	...
	if (buscarCarta (LC, idCartaModi, anioCartaModi) ==0 ){
		printf("Debe ingresar un id del nino/nina valido o anio valido");
		return;
	}
	...
	if(modiLisSe == 1){
		...
		if (BuscarJ(root, modiJugue) == 0){
		printf("No se encontro el juquete");
		return;
		}
		modificarCarta (LC, idCartaModi, anioCartaModi, modiSelec, modiJugue );
		
	}
	else if ( modiLisSe == 2){
		mostrarListaCarta (LC);
		printf("Ingrese el nombre de juguete para agregar: ");
		gets (modiJugue);
...
void consultarCarta (const ListaCartaNino *LC ){
	int idConsultaC, anioConsultaC;
	NodoCarta *i;
	
	printf("Ingrese el numero de cedula del nino/nina a consultar carta: ");
	scanf("%d",&idConsultaC);
	getchar(); //borrar basura
	
	printf("Ingrese el anio de la carta correspondiente del nino/nina: ");
	scanf("%d",&anioConsultaC);
	getchar(); //borrar basura
	
	if ( buscarCarta (LC, idConsultaC, anioConsultaC) == 0){
		printf("Debe ingresar un id del nino/nina valido o anio valido");
		return;
	}
	else{
		for (i=LC->inicio; i!= NULL; i=i->siguiente){
		if(i->idNinoC == idConsultaC && i->anio == anioConsultaC){
			printf("ID Nino/Nina: %d |Nombre Nino/Nina: %s | Anio: %d | Estado: %s", i->idNinoC, i->nombreNInoC, i->anio,i->estado); 
			printf("LISTA JUGUETES: 1.%s , 2.%s, 3.%s, 4.%s, 5.%s, 6.%s, 7.%s, 8.%s, 9.%s, 10.%s\n", i->juguete1, i->juguete2, i->juguete3, i->juguete4, i->juguete5, i->juguete6, i->juguete7, i->juguete8, i->juguete9, i->juguete10);
			}
		free(i);
		return;
...
void quitarJuguete (ListaCartaNino *LC){
	int idQuitaC, anioQuitaC, pModiSelec;
	NodoCarta *i;
	...
	if ( buscarCarta (LC, idQuitaC, anioQuitaC) == 0){
		printf("Debe ingresar un id del nino/nina valido o anio valido");
		return;
	}
	else{
		for (i=LC->inicio; i!= NULL; i=i->siguiente){
		if(i->idNinoC == idQuitaC && i->anio == anioQuitaC){
			printf("LISTA JUGUETES: 1.%s , 2.%s, 3.%s, 4.%s, 5.%s, 6.%s, 7.%s, 8.%s, 9.%s, 10.%s\n", i->juguete1, i->juguete2, i->juguete3, i->juguete4, i->juguete5, i->juguete6, i->juguete7, i->juguete8, i->juguete9, i->juguete10);
			
			printf("\nIngrese el numero de juguete a quitar");
			scanf("%d",&pModiSelec);
			getchar();
...
//                                       TODO SOBRE REGISTRO DE PROCESAMIENTO
//-----------------------------------------------------------------------------------------------------------------

//Declara una lista doble

ListaPro *listaNuevaPro (void) {
	ListaPro *L;
	L = (ListaPro *) malloc(sizeof(ListaPro));
	L->inicio = NULL;
	L->final = NULL;
...
NodoPro* crearNodoPro( int idAyudantPro, int aniPro, int idNinPro, const char pDecision[] ){
	NodoPro *nuevo;
	nuevo = (NodoPro *) malloc(sizeof(NodoPro));
	nuevo->anterior = NULL;
	nuevo->siguiente = NULL;
	
	nuevo->idAyudantePro= idAyudantPro;
	nuevo->anioPro = aniPro;
	nuevo->idNinoPro = idNinPro;
	
	strcpy (nuevo->decision,pDecision);
	return nuevo;
}

//Inserta un dato a una lista doble

void insertarDatoPro(ListaPro *L, int idAyudanPro, int anPro, int idNiPro, const char qDecision[])
{
	if(L->inicio == NULL) 
	{
	
		L->inicio = crearNodoPro(idAyudanPro, anPro, idNiPro, qDecision);
		L->final = L->inicio;
		return;
	}
	L->final->siguiente = crearNodoPro(idAyudanPro, anPro, idNiPro, qDecision);
	L->final->siguiente->anterior = L->final;
	L->final = L->final->siguiente;
}

//Funcion para msotrar una lista doble
void mostrarListaPro(const ListaPro *L)
{
	NodoPro *i;
	
	for(i = L->inicio; i!= NULL; i = i->siguiente){
		printf("Id_Ayudante: %d |Anio: %d |Id_Nino/Nina: %d |Decision: %s\n", i->idAyudantePro, i->anioPro, i->idNinoPro, i->decision);
	}
	free(i);
	printf("\n\n");
}
...
//Funcion para liberar una lista doble
void liberarListaPro(ListaPro *L)
{
	NodoPro *n, *aux;
...
//FUNCION QUE RETORNA LA CANTIDAD DE COMPORTAMIENTO
int cantidadComportamiento (const ListaC *C, int idCom ){
	NodoC *aux;
	
	for(aux=C->inicio; aux!=NULL; aux = aux->siguiente){
		if (aux->IDnino == idCom){
			return aux->contador_comport;
		}
	}
}

//FUNCION QUE ENVIA MENSAJE A LOS PAPAS
void mensajePapas (const ListaC *C, int idCom){
	NodoC *aux;
	
	for(aux=C->inicio; aux!=NULL; aux = aux->siguiente){
		if (aux->IDnino == idCom){
			if(aux->contador_comport>6){
				printf("Estimados padres de familia: %s y %s su hijo/a se ha portado mal. No recibira regalos de navidad", aux->nombreMadre, aux->nombrePadre);
			}
		}
	}
}

//FUNCION QUE MUESTRA LAS CARTAS DE UN ANIO DADO
void mostrarCartaAnio (const ListaCartaNino *LC, int anioC){
	NodoCarta *auxi;
	
	for (auxi = LC->inicio; auxi!=NULL; auxi=auxi->siguiente){
		if(auxi->anio == anioC){
			printf("ID NINO/NINA: %s | NOMBRE: %s", auxi->idNinoC, auxi->nombreNInoC);
		}
	}
}

//FUNCION QUE RETORNA SI EXISTE UNA CARTA CON EL ANIO DADO
int existeAnio (const ListaCartaNino *LC, int anioC){
	NodoCarta *auxi;
	
	for (auxi = LC->inicio; auxi!=NULL; auxi=auxi->siguiente){
		printf("Anio: %d y compara %d", auxi->anio, anioC);
		if(auxi->anio == anioC){
			return 1;
		}
	}
...
//Funcion para agregar Ayudantes de santa 
ListaPro *agregarProcesamiento (ListaPro *LD, const ListaDobleAS *Lista, const ListaCartaNino *LC, const ListaC *C ){
	
	int idAyuPro, aniOPro, idNinAPro;
	char qDecision [LONG];
	NodoCarta *i;
	
	printf ("-------- Ingrese los datos para el procesamiento -------- \n");
	
	printf("\nIngrese la identificacion del ayudante\n: ");
	scanf("%d",&idAyuPro);
	getchar ();
	
	if (esIdValido (Lista,idAyuPro ) == 0){
		printf("Debe ingresar un id de ayudante valido");
		return;
	}
	
	printf("\nIngrese el anio de la carta a procesar: ");
	scanf("%d",&aniOPro);
	getchar ();
	
	if (existeAnio (LC,aniOPro) == 0){
		printf("Debe ingresar un anio valido");
		return;
	}
	mostrarCartaAnio (LC,aniOPro);
	
	printf("\nIngrese la identificacion del nino de la carta: ");
	scanf("%d",&idNinAPro);
	getchar ();
	

	if ( buscarCarta (LC, idNinAPro, aniOPro) == 0){
		printf("Debe ingresar un id del nino/nina valido o anio valido");
		return;
	}
	
	for (i=LC->inicio; i!= NULL; i=i->siguiente){
		if(i->idNinoC == idNinAPro && i->anio == aniOPro){
			printf("ID Nino/Nina: %d |Nombre Nino/Nina: %s | Anio: %d | Estado: %s", i->idNinoC, i->nombreNInoC, i->anio,i->estado); 
			printf("LISTA JUGUETES: 1.%s , 2.%s, 3.%s, 4.%s, 5.%s, 6.%s, 7.%s, 8.%s, 9.%s, 10.%s\n", i->juguete1, i->juguete2, i->juguete3, i->juguete4, i->juguete5, i->juguete6, i->juguete7, i->juguete8, i->juguete9, i->juguete10);
			}
	...
	for (i=LC->inicio; i!= NULL; i=i->siguiente){
		if(i->idNinoC == idNinAPro && i->anio == aniOPro){
			if( cantidadComportamiento (C, idNinAPro) <= 6){
				strcpy(i->estado, "Listo por entregar");
			}
			else{
				strcpy(i->estado, "Rechazado");
				mensajePapas (C,idNinAPro );
			}
			return;
		}
	}
	insertarDatoPro (LD,idAyuPro, aniOPro, idNinAPro, i->estado);
	}
	free(i);
	return LD;
}
...
char* quitarlinea(char linea[]){  //Al cargar la persistencia se cargan con un salto de linea, hay que quitarlo para que se guarde en las variables correctamente, ya que las sacamos por cada linea
    int i;                                   //Referencia: https://stackoverrun.com/es/q/579413
    for(i=0; linea[i]!='\0'; i++)   
        if(linea[i] == '\n')
            linea[i] ='\0';
	return linea;
...
void Cargar_Persistencia_ninos(ListaR *L){
  ...
  int Econversion;
	...
	FILE* RegistroNinos;    
    RegistroNinos = fopen("RegistroNinos.txt", "r");  //Se abre el txt miembro


	while (!feof(RegistroNinos)){   //mientras no este vacio, saca informacion de cada linea y lo guarda en una variable
		fgets(identificacion,100,RegistroNinos);
		quitarlinea(identificacion);
		fgets(nombre,100,RegistroNinos);
		quitarlinea(nombre);
		fgets(N_usuario,100,RegistroNinos);
		quitarlinea(N_usuario);
		fgets(lugar,100,RegistroNinos);
		quitarlinea(lugar);
		fgets(correo,100,RegistroNinos);
		quitarlinea(correo);
		fgets(fecha_nacimiento,100,RegistroNinos);
		quitarlinea(fecha_nacimiento);
		fgets(edad,100,RegistroNinos);
		quitarlinea(edad);
		fgets(necesidades_especiales,100,RegistroNinos);
		quitarlinea(necesidades_especiales);
		
		//conversion
		Iconversion= atoi(identificacion);
		Econversion= atoi(edad);
		
		insertarNino(L,Iconversion,nombre,N_usuario,lugar,correo,fecha_nacimiento,Econversion,necesidades_especiales);  //llama a la funcion insertar con los datos sacados del txt
	}
}

void Guardar_Persistencia_ninos(ListaR *L)  //se guarda en txt miembros
{
	FILE* RegistroNinos; 
	NodoR *i;   
    RegistroNinos = fopen("RegistroNinos.txt", "w"); 
	
	char Iconversion[100];
	char Econversion[100];


		for(i = L->inicio; i!= NULL; i = i->siguiente)
		{
			                                      //imprime en orden los datos de la lisat de miembros
			int id=i->identificacion;
    		itoa(id,Iconversion,10);   // decimal
    		fprintf(RegistroNinos,"%s\n", Iconversion);
			...
			int ed=i->edad;
    		itoa(ed,Econversion,10);   // decimal
    		fprintf(RegistroNinos,"%s\n", Econversion);
...
void contarEstados ( const ListaCartaNino *LC){
	int rechazada = 0;
	int aprobada = 0;
	NodoCarta *i;
	
	for (i=LC->inicio; i!=NULL; i=i->siguiente){
		if( strcmp(i->estado, "Rechazada") == 0){
			rechazada++;
		}
		else{
			aprobada++;
	...
	printf("La cantidad de ninos con cartas rechazadas es de %d y de aprobadas es de %d", rechazada, aprobada);
}
...
int main(void)
{
	...
	Cargar_Persistencia_ninos(L);
	
	//Instancia de registro de procesamiento
	ListaPro *LPRO;
	LPRO = listaNuevaPro ();
	
	menu(L,C,root, Lista, LiCa, LPRO);
}
	
int menu(ListaR *L,ListaC *C,struct node* root, ListaDobleAS *Lista, ListaCartaNino *LiCa, ListaPro *LPRO){
	...
	printf("23. Modificar un juguete de una carta para santa\n");
	printf("24. Quitar un juguete de una carta para santa\n");
	printf("25. Consultar una carta para santa\n");
	printf("26. Mostrar listas de carta para santa y deseos\n\n");
	
	printf("27. Procesar una carta para santa\n\n");
	
	printf("28. Entregar juguetes\n\n");
	
	printf("29. Analisis de datos\n\n");
...
//	--------------------------------------Eliminar juguetes del arbol--------------------------------------    
    if (strcmp(opcion,"11")==0){
		...
		printf("\nQue nombre exacto tiene el juguete que desea eliminar \n\n");
 			gets(nombre);  
 ...
 //--------------------------------------Mostrar lista de juguetes------------------------------------- 	
  ...
  if (strcmp(opcion,"13")==0){
	  ...
	  int contador_comport;
        ...
        insertarNinoC(C,IDnino,nombrePadre,nombreMadre,Des_comportamiento,fecha_Registro,comportamiento,contador_comport);
 ...
 //--------------------------------------Eliminar una ruta------------------------------------- 	
  ...
  //--------------------------------------Mostrar catalogo de domicilios------------------------------------- 	
    ...
    //--------------------------------------Modificar una juguete de carta para santa------------------------------------- 	
  	if (strcmp(opcion,"23")==0){
        		editarCarta (LiCa, root);
           		printf("\nPresione ENTER para continuar.\n"); 
				getch();
				menu(L,C,root, Lista, LiCa, LPRO);
    }
    
    //--------------------------------------Quitar un juguete de una carta para santa------------------------------------- 	
  	if (strcmp(opcion,"24")==0){
        		quitarJuguete (LiCa);
           		printf("\nPresione ENTER para continuar.\n"); 
				getch();
				menu(L,C,root, Lista, LiCa, LPRO);
    }
    //--------------------------------------Consultar una carta para santa------------------------------------- 	
  	if (strcmp(opcion,"25")==0){
        		consultarCarta (LiCa);
           		printf("\nPresione ENTER para continuar.\n"); 
				getch();
				menu(L,C,root, Lista, LiCa, LPRO);
    ...
    //--------------------------------------Mostrar listas de carta para santa y deseos------------------------------------- 	
  	if (strcmp(opcion,"26")==0){
        		mostrarListaCarta(LiCa);
           		printf("\nPresione ENTER para continuar.\n"); 
				getch();
				menu(L,C,root, Lista, LiCa, LPRO);
    }
    
    //--------------------------------------Procesar una carta para santa y deseos------------------------------------- 	
  	if (strcmp(opcion,"27")==0){
        		LPRO = agregarProcesamiento (LPRO, Lista, LiCa, C );
           		printf("\nPresione ENTER para continuar.\n"); 
				getch();
				menu(L,C,root, Lista, LiCa, LPRO);
    }
    ...
    //--------------------------------------Analisis de datos------------------------------------- 	
  	if (strcmp(opcion,"29")==0){
        		printf("Cantidad de juguetes por anio\n");
        		printf("2021: %d", LiCa->cantidadJuguetes);
        		
        		printf("Lugar donde se solicitaron mas juguetes\n");
        		
        		printf("Cantidad de ninos con carta rechazada y aprobada\n");
        		printf("Para el 2021: \n");
				contarEstados (LiCa);
				
				printf(" Cantidad de comportamientos buenos y malos\n");
				
				printf("Cantidad de cartas procesadas por ayudante: \n");
    ...
    //---------------------------------------SALIR DEL PROGRAMA----------------------------------------------
    if (strcmp(opcion,"30")==0){
    	
    }
	  
	//escribirAyudantes (Lista);
	//liberarListaAS (Lista);
